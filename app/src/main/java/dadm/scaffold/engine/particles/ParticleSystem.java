...
public class ParticleSystem extends ScreenGameObject {
    ...
    public ParticleSystem setSpeedRange(double speedMin, double speedMax) {
        mInitializers.add(new SpeedModuleAndRangeInitializer(speedMin*mPixelFactor, speedMax*mPixelFactor, 0, 360));
        return this;
    }

    public ParticleSystem setSpeedModuleAndAngleRange(double speedMin, double speedMax, int minAngle, int maxAngle) {
        mInitializers.add(new SpeedModuleAndRangeInitializer(speedMin*mPixelFactor, speedMax*mPixelFactor, minAngle, maxAngle));
        return this;
    }

    public ParticleSystem setSpeedByComponentsRange(double speedMinX, double speedMaxX, double speedMinY, double speedMaxY) {
        mInitializers.add(new SpeedByComponentsInitializer(speedMinX*mPixelFactor, speedMaxX*mPixelFactor,
    ...
    public ParticleSystem addInitializer(ParticleInitializer pi){
        mInitializers.add(pi);
        return this;
    }
    ...
    private void activateParticle(GameEngine gameEngine) {
        ...
        p.activate(gameEngine, mTimeToLive, positionX, positionY, mModifiers);
        mActivatedParticles++;
    }
